...
class MainClass {
  public static void Main (string[] args) {
    ...
    todoList.Insert(new Todo(0, 10, "Game")); // 0: 아이디, 10: 우선순위
  ...
  public class Todo : IComparable, IEnumerable<Todo> {
    public int id;
    public int priority;
    public string desc;
    public Todo(int id, int priority, string dese) {
      this.id = id;
      this.priority = priority;
      this.dese = desc;
    }
      public int ComareTo(object other) {
  return priority.ComareTo(((Todo)other).priority);
  }
...
public class MinHeap<T> where T: IComparable, IEnumerable<T> {
  List<T> list;
  ...
  void HeapifyDown(int p) {
    ...
    if(r > list.Count-1) {
      // if(list[l] < list[p])
      if(list[l].ComareTo(list[p] ) < 0){
        list.Swap(l, p);
        return;
    } else {
      // if(list[r] <= list[p]) {
        if(list[l].ComareTo(list[r]) <= 0){
        // if(list[l] < list[p]) {
          if(list[l].ComareTo(list[p]) < 0){
          list.Swap(r, p);
          HeapifyDown(r); 
  ...
  // Func<int,bool> f
  public int Find(Predicate<T> f) { // return Predicate은 제한이 없고 리턴값이 트루 펄스 인 리턴값이 1개인 것 == Func<int,bool> f  같다
