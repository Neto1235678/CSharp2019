...
class MainClass {
  ...
  public static void FloodFill(int[][] image, int StartRow, int StartColum, int replacecolor) {
    int width = image[0].Length;
    int height = image.Length;

    if(!(0 <= StartColum && StartColum < width)) 
      return;
    if(!(0 <= StartRow && StartRow < height)) 
      return;     

    int targetColor = image[StartRow][StartColum];
    Queue<Pos> q = new Queue<Pos>();
    q.Enqueue(new Pos(StartRow, StartColum));
    ...
    while(q.Count > 0) {
      ...
      if(y+1 < width && x+1 < height && image[x+1][y+1] == targetColor)
          q.Enqueue(new Pos(x+1,y+1));
